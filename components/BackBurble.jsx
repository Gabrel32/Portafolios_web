...
const BackBurble = ({
  ...
  return (
    <div
      ref={containerRef}
      className="relative overflow-hidden w-full min-h-screen py-16 px-2 bg-transparent dark:from-gray-900/50"
    >
      {/* Fondo de partículas */}
      <div className="absolute inset-0 -z-70 w-full h-full">{particles}</div>

      {/* Contenido principal */}
      <div
        className={`relative w-full flex flex-col h-full items-center justify-center ${
          variant === "wide" ? "max-w-7xl" : "max-w-4xl"
        } mx-auto px-4`}
      >
        {title && (
          <div
            className={`inline-block relative ${center ? "text-center" : ""}`}
          >
            <h1
              className={`text-4xl md:text-5xl font-bold tracking-tight text-custom-brown
            ...
            {showLine && (
              <div
                className={`absolute ${center ? "left-1/2 -translate-x-1/2" : "left-0"}
        ...
        {description && (
          <p
            className={`mt-4 text-xl md:text-2xl text-primary dark:text-completColor 
        ...
        {children && (
          <div
            className={`relative mt-8 w-full ${center ? "flex justify-center" : ""}`}
          >
            {children}
          </div>
        )}
