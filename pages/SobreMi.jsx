        ...
        <section className="mb-10 relative z-10">
          ...
          <ul className='mt-6 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
            {tecnologias.map((e, index) => (
              <li
                key={e.id}
                className={`flex flex-col items-center p-6 bg-transparent rounded-2xl shadow-lg hover:shadow-xl dark:shadow-[#f5f5f56e] transition-all duration-300 hover:-translate-y-2 animate-cardPop delay-${index + 1}`}
              ...
              >
                ...
                <span className='text-lg font-medium text-gray-800 dark:text-[#f5f5f5] relative'>
                  {e.Nombre}
                  <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#DC5F00] transition-all duration-300 group-hover:w-full"></span>
                </span>
              </li>
            ))}
          </ul>
